...
static int isp_submit_buffers(struct apple_isp *isp)
{
	...
	while ((buf = list_first_entry_or_null(&isp->bufs_pending,
		...
		for (int j = 0; j < fmt->num_planes; j++) {
			bufd->iovas[j] =
				vb2_dma_contig_plane_dma_addr(&buf->vb.vb2_buf, 0) + offset;
...
static int isp_vb2_buf_init(struct vb2_buffer *vb)
{
	...
	for (i = 0; i < vb->num_planes; i++)
		buf->surfs[i].iova = vb2_dma_contig_plane_dma_addr(&buf->vb.vb2_buf, i);
...
int apple_isp_setup_video(struct apple_isp *isp)
{
	...
	err = vb2_dma_contig_set_max_seg_size(isp->dev, DMA_BIT_MASK(32));
	if (err) {
		dev_err(isp->dev, "Failed to set vb2 dma max seg size: %d\n", err);
